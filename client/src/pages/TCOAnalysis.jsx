...
export default function TCOAnalysis() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* PM Schedule Assignment Banner */}
      {selectedScenario && !selectedScenarioPmId && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle size={20} className="text-amber-600 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-amber-800">PM Schedule Required</h3>
              <p className="text-sm text-amber-700 mt-1">
                This scenario needs a PM schedule to include maintenance costs in the analysis. Create schedules in{' '}
                <a href="#" onClick={(e) => { e.preventDefault(); window.open('/pm-planner', '_blank'); }} className="font-medium underline hover:text-amber-900">
                  PM Planner
                </a>{' '}
                and assign them here.
              </p>
              <div className="flex items-center gap-3 mt-3">
                <button onClick={openAssignPmModal} className="btn btn-primary btn-sm">
                  Assign PM Schedule
                </button>
                <a href="/pm-planner" target="_blank" rel="noopener noreferrer" className="text-sm text-amber-700 hover:text-amber-900 font-medium">
                  Open PM Planner →
                </a>
              </div>
            </div>
      ...
      <Modal open={scenarioModalOpen} onClose={() => setScenarioModalOpen(false)} title="New Analysis Scenario" size="lg">
        <form onSubmit={handleCreateScenario} className="space-y-5">
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="mb-3">
              <h3 className="text-sm font-medium text-gray-700">Labor Rates ($/hr by skill level)</h3>
              <p className="text-xs text-gray-500 mt-1">Set hourly rates for different technician skill levels. These rates affect labor cost calculations.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Entry-level maintenance tasks">Basic</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Standard maintenance and repairs">Intermediate</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Complex diagnostics and major repairs">Advanced</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Specialized equipment and OEM work">Specialist</label>
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="mb-3">
              <h3 className="text-sm font-medium text-gray-700">Financial Parameters</h3>
              <p className="text-xs text-gray-500 mt-1">Configure economic assumptions and operational parameters for accurate cost projections.</p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Discount applied to parts pricing">Parts Discount (%)</label>
                <input type="number" className="input" value={scenarioForm.parts_discount} onChange={(e) => updateField('parts_discount', e.target.value)} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Additional markup for overhead costs">Overhead Markup (%)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Total years to analyze costs over">Analysis Period (yrs)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Rate used to discount future costs to present value">Discount Rate (%)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Expected annual increase in costs">Inflation Rate (%)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Current fuel cost per liter">Fuel Cost ($/L)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Cost per hour of equipment downtime">Downtime Cost ($/hr)</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Number of working days per year">Working Days/Year</label>
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Average hours worked per day">Hours/Day</label>
                <input type="number" className="input" value={scenarioForm.hours_per_day} onChange={(e) => updateField('hours_per_day', e.target.value)} />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1" title="Expected equipment utilization percentage">Utilization Target (%)</label>
